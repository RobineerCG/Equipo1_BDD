...
select * from dbo.copiacovid where 
CLASIFICACION_FINAL between 1 and 3
order by ENTIDAD_RES;
...
select top 5 c.CasosSC, c.MUNICIPIO_RES, c.ENTIDAD_RES from (select MUNICIPIO_RES, ENTIDAD_RES, count(CLASIFICACION_FINAL) CasosSC from dbo.copiacovid 
where CLASIFICACION_FINAL between 1 and 3 or CLASIFICACION_FINAL=6
group by ENTIDAD_RES, MUNICIPIO_RES) as c;
...
select TOP 1 MUNICIPIO_RES, count(FECHA_DEF) as DefuncionesCC from dbo.copiacovid
where FECHA_DEF !='9999-99-99' and CLASIFICACION_FINAL between 1 and 3
group by MUNICIPIO_RES order by DefuncionesCC desc;
...
--Casos Sospechosos
count(case CLASIFICACION_FINAL when 6 then CLASIFICACION_FINAL end)as sospechosos
from dbo.copiacovid group by ENTIDAD_RES) ccs
inner join
--Casos confirmados de MARZO A AGOSTO 2020
(select ENTIDAD_RES, count(FECHA_DEF) TDefuncionesMA from dbo.copiacovid where FECHA_DEF between '2020-03-01' and '2020-08-31' group by ENTIDAD_RES)
dma
on dma.ENTIDAD_RES=ccs.ENTIDAD_RES
join
--Casos confirmados de DICIEMBRE 2020 A MAYO 2021
(select ENTIDAD_RES, count(FECHA_DEF) TDefuncionesDM from dbo.copiacovid where FECHA_DEF between '2020-12-01' and '2021-05-31' group by ENTIDAD_RES) ddm
on dma.ENTIDAD_RES=ddm.ENTIDAD_RES;
...
select @var1=1, @var2=1, @var3=1;
--Defunciones en menores de edad en 2020
select @var1= count(FECHA_DEF) from dbo.copiacovid where edad<18 and FECHA_DEF between '2020-01-01' and '2020-12-31'
--Defunciones en Menores de edad en 2021
select @var2= count(FECHA_DEF) from dbo.copiacovid where edad<18 and FECHA_DEF between '2021-01-01' and '2021-12-31'
--Defunciones en Menores de edad en 2022
select @var3= count(FECHA_DEF) from dbo.copiacovid where edad<18 and FECHA_DEF between '2022-01-01' and '2022-12-31'
select 'Verdadero' as 'En 2020 hay mas defunciones en -18' where @var1>@var2 and @var1>@var3;
...
declare @var4 float, @var5 float
select @var4=count(RESULTADO_LAB)*0.6 from dbo.copiacovid where RESULTADO_LAB =1  and FECHA_ACTUALIZACION between '2021-01-01' and '2021-12-31'
select @var5= count(RESULTADO_LAB)*0.6 from dbo.copiacovid where RESULTADO_LAB=1 and FECHA_ACTUALIZACION between '2020-01-01' and '2020-12-32'
...
declare @var6 int, @var7 int, @var8 int, @var9 int,@total int;
select @var6=count(TIPO_PACIENTE) from dbo.copiacovid where TIPO_PACIENTE=1 and EDAD < 18;
Select @var7= count(TIPO_PACIENTE)from dbo.copiacovid where TIPO_PACIENTE=1 and EDAD between 19 and 40;
select @var8= count(TIPO_PACIENTE)from dbo.copiacovid where TIPO_PACIENTE=1 and EDAD between 40 and 60;
select @var9= count(TIPO_PACIENTE)from dbo.copiacovid where TIPO_PACIENTE=1 and EDAD >60;
IF @var6>@var7 and @var6>@var8 and @var6>@var9
begin
print 'Casos reportados y recuperados: Menores de edad'
end else
